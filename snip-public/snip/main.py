...
def main():
    ...
    # mode = 'generate_masks_ci'
    # mode = 'generate_masks'
    # mode = 'calc_untrained_accuracy'
    mode = 'convergence'
    ...
    if mode == 'generate_masks':
        for seed in range(1000):
            tf.reset_default_graph()
            tf.set_random_seed(seed)

            # Model
            model = Model(num_classes=dataset.num_classes, **vars(args))#, mask_file='masks_lenet300/threshold_300')
            ...
            # Prune
            prune.prune(args, model, sess, dataset, save_mask=True, sparsity=args.target_sparsity, seed=seed, file_prefix=args.initializer_w_bp, arch=args.arch, data='cifar')

            sess.close()
        sys.exit()
    elif mode == 'generate_masks_ci':
        for seed in range(1000):
            tf.reset_default_graph()
            tf.set_random_seed(seed)
            sess = tf.InteractiveSession()
            ...
            # Prune
            prune.prune(args, model, sess, dataset, save_mask=False, save_mask_ci=True, sparsity=args.target_sparsity, seed=seed, file_prefix=args.initializer_w_bp, arch=args.arch)

            sess.close()
        sys.exit()

    elif mode == 'calc_untrained_accuracy':
        for seed in range(100):
            tf.reset_default_graph()
            tf.set_random_seed(seed)

            # Model
            model = Model(num_classes=dataset.num_classes, **vars(args))#, mask_file='masks_lenet5_mnist/random_1')
            model.construct_model()
            ...
            # Prune
            prune.prune(args, model, sess, dataset, save_mask=False, sparsity=args.target_sparsity, seed=seed, file_prefix=args.initializer_w_bp, arch=args.arch)


            train.train(args, model, sess, dataset)
            errors.append(test.test(args, model, sess, dataset))
        ...
        # Model
        model = Model(num_classes=dataset.num_classes, **vars(args), mask_file='masks_alexnetv2/random2') #'masks_lenet5_mnist/random_3')
        model.construct_model()
        ...
        # Prune
        # prune.prune(args, model, sess, dataset, save_mask=False, sparsity=args.target_sparsity, seed=seed, file_prefix='______' + args.initializer_w_bp, arch=args.arch)
