...
class Conv2_rank1(nn.Module):
    def __init__(self):
        super(Conv2_rank1, self).__init__()
        builder = get_builder()
        self.convs = nn.Sequential(
            builder.conv3x3(3, 64, first_layer=True),
            nn.ReLU(),
        ...
        self.linear = nn.Sequential(
            builder.conv1x1_rank1(64 * 16 * 16, 256),
            nn.ReLU(),
            builder.conv1x1(256, 256),
            nn.ReLU(),
            builder.conv1x1(256, 10),
...
class Conv4_rank1(nn.Module):
    def __init__(self):
        super(Conv4_rank1, self).__init__()
        builder = get_builder()
        self.convs = nn.Sequential(
            builder.conv3x3(3, 64, first_layer=True),
            nn.ReLU(),
